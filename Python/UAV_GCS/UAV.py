...
class DroneController_flask:
    def __init__(self, sio):
        ...
        self.filename = "mission.txt"
        self.goto_mission = "waypoints.txt"
        self.FLASK_SERVER = "http://127.0.0.1:5000/telemetry"
    ...
    def send_telemetry_data(self):
        while True:
            ...
            try:
                try:
                    response = requests.get(self.FLASK_SERVER)
                    if response.status_code == 200:
                        telemetry_data = response.json()
                        telemetry_data["locations"] = locations
                        self.sio.emit('telemetry', telemetry_data, namespace="/python")
                        print("Telemetry Data:", telemetry_data)
                    else:
                        print(f"Failed to retrieve telemetry: {response.status_code}")
                except Exception as e:
                    print(f"Error in retrieving telemetry data: {str(e)}")
                time.sleep(1)
            except Exception as e:
                print("UAV 1 not connected")
                # self.uav_connected=False
