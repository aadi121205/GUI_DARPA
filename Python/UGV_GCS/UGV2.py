...
class RoverController2:
    def __init__(self, sio):
        ...
        self.goto_mission = "waypointsr2.txt"
        ...
        self.connect_ugv()
        threading.Thread(target=self.send_telemetry_data_rover2, daemon=True).start()
            ...
            try:
                if self.ugv_connected and not self.ugv_connection._heartbeat_timeout:
                    try:
                        ...
                        self.sio.emit(
                            "telemetry_rover2", telemetry_data, namespace="/rover2"
                        )
                    except Exception as e:
